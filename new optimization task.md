...
# Create a new optimization task for SORTD
...
5. Add your new task to every .cpp file that implements SORTD's templates. E.g., `src/solver/solver.cpp`, `src/utils/file_reader.cpp`, etc.
...
8. Check if the code compiles and run SORTD with `-task my-new-task` to see if it runs properly. You are now ready to modify the new task as you wish!
...
Implicitly defined: 
* `SolContainer` or equivalently `Node<MyNewTask>`: The data type of information on the solution (e.g., solution value, feature, label).
...
* `element_additive` (`bool`): `True` if the solution values are element-wise additive. This means SORTD can use its similarity lower bound. `False` disables the similarity lower bound.
...
* `minimum_difference` (`SolType`): The minimum difference between two non equivalent solutions (e.g., 1 for misclassification score). This is used to compute an upper bound from a given solution.
...
4. Add a case `my_new_task` to `initialize_sortd_solver` and call `Solver<MyNewTask>`.
...
2. Define the `__init__` constructor and provide default parameters. Pass all paramters except newly defined parameters to `BaseSORTDSolver`.
